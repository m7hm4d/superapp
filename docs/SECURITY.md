...
2. **مفتوح في إعدادات GitHub — `contract` ليس required check.** يجب إضافته بعد ظهور الاسم النهائي للفحص على PR ناجح.

3. **مفتوح — لا توجد بيئة `production` محمية.** workflow يمكنه الإشارة إليها في الشيفرة، لكن المراجع ومنع self-review وفروع النشر والأسرار كلها إعدادات يدوية.
...
1. **مفتوح في إعدادات GitHub — لا يفرض linear history.** الوثائق تطلب squash بينما GitHub يسمح أيضاً بالـmerge والـrebase.
