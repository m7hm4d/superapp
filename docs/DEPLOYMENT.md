...
يتحقق التشغيل أن Git tag موجود وأن commit الخاص به ancestor لـ`main` وأن بوابات ذلك SHA نجحت. الترقية تصلح فقط لـcommits دخلت خط الأنابيب الحالي: commit يسبق وجود `security.yml` أو `publish.yml` لا يملك بوابة مكتملة ولا صورة `sha-<40-hex>`، وتفشل ترقيته فوراً برسالة تشرح السبب بدل انتظار مهلة البوابات؛ العلاج هو إصدار commit أحدث من `main` (أو revert للأمام) لا محاولة إحياء commit قديم. بعد ذلك يتحقق read-only من صورتي `sha-<40-hex>`: الـdigest وOCI revision وتوقيع `main` وworkflow SHA، ثم يعيد قراءة Git tag من GitHub مباشرة قبل إنشاء GitHub Release. لا ينشئ `vX.Y.Z` أو أي SemVer alias في GHCR؛ ملاحظات الإصدار تسجل الـdigestين فقط، والنشر يستعمل SHA الكامل. إعادة التشغيل تقبل GitHub Release الموجود فقط إذا طابق الاسم وGit tag والمصدر والنص والـdigests ولم يحوِ assets؛ وإلا تفشل بلا overwrite.
...
4. يختار هدف العودة الآلية من آخر سطر في `<stack>-history.tsv` — آخر نشر اجتاز كل الفحوص — لا مما يصادف أنه يعمل الآن، لأن نشراً فاشلاً يترك حاويات الإصدار المكسور عاملة. عند غياب السجل (أول تشغيل بعد الترقية أو bootstrap) يلتقط RepoDigests الفعلية للحاويتين العاملتين ويشترط أنهما من SHA واحد. في الحالتين يتحقق من توقيع الهدف بثلاث محاولات مع إظهار خطأ cosign عند الفشل؛ وعند فشل نهائي (هوية توقيع قديمة أو انقطاع sigstore طويل) يمكن النشر بلا عودة آلية عبر `DEPLOY_ALLOW_UNVERIFIED_PREVIOUS=1` بدل بقاء كل النشرات مقفلة حتى تُزال الحاويات يدوياً.
...
${DEPLOY_STATE_DIR}/runs/<sha>-<run>-<attempt>/   # ينشئه workflow الإنتاج فقط
```

installer الإنتاج يطبق احتفاظاً محدوداً في كل نشر: يحذف مجلدات `runs/` الأقدم من 30 يوماً (المراقبة تتوقف بعد 75 دقيقة فلا يكون فيها نشر حي)، وbundles `.releases/` غير الحالية الأقدم من 30 يوماً، وأشجار التثبيت المقطوعة (المجلدات النقطية في `.releases/`) الأقدم من يوم بعد إعادة صلاحية الكتابة إليها. من دون هذا كان النمو غير المحدود ينتهي بفشل حارس المساحة في `backup-db.sh` بسبب بعيد عن مصدره.

قبل dump يقيس السكربت حجم قاعدة البيانات والمساحة الحرة، ويشترط ضعف الحجم مع هامش `512 MiB`. وبعد اكتمال النسخة الجديدة والتحقق من فهرستها يدوّر ما تجاوز `MAX_LOCAL_BACKUPS=30` افتراضياً بحذف الأقدم فالأقدم مع تسجيل كل حذف؛ فلا يتوقف النشر الآلي عند بلوغ الحد، ولا تُحذف نسخة قبل وجود بديل أحدث سليم. النقل خارج الخادم واختبار restore يبقيان مسؤولية تشغيلية مستقلة.
