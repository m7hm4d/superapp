...
- أدمن اللوحة: `admin@superapp.local` — كلمة المرور تأتي من `SEED_ADMIN_PASSWORD` عند تشغيل الـ seed، وإن لم تُعيَّن تُولَّد عشوائياً وتُطبع مرة واحدة في مخرجات الـ seed (لا توجد كلمة مرور افتراضية منشورة). دخول الأدمن حصراً من لوحة الويب (`/auth/admin/login` + TOTP بعد تفعيله) — مسار الهاتف `/auth/login` يرفض حسابات الأدمن.
